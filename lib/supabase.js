...
export async function uploadRecording(userId, date, recordingFile, title = null) {
  ...
  try {
    ...
    console.log('Uploading recording:', fileName, 'Type:', recordingFile.type, 'Size:', recordingFile.size);
    ...
    if (upErr) {
      console.error('Recording storage upload error:', upErr);
      throw new Error(`Recording upload failed: ${upErr.message}`);
    }

    console.log('Recording uploaded to storage:', uploadData);

    const { data: { publicUrl } } = supabase.storage
      .from('study-recordings')
      .getPublicUrl(fileName);

    console.log('Recording public URL:', publicUrl);
    ...
    console.log('Inserting recording into database:', recordingData);
    ...
    if (error) {
      console.error('Recording database insert error:', error);
...
export async function getAllRecordings() {
  ...
  console.log('All recordings fetched:', data?.length || 0, 'recordings');
  return data;
}